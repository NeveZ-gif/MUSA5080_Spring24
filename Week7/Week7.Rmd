...
```
Note: Notice that the coefficient for displacement is very different from the correlation test observation. The issue has probably arisen due to multicolinearity.
...
The residuals seem a bit abnornormal (non-normal). 
Note: Q-Q plot seems off at the higher end. The same pattern is observed in Residuals vs. Fitted plot.
...
Note: This is when we start determining if a model is for explanation or prediction.
...
```{r tinkering with data}
library(sf)
library(dplyr)
Auto.1 <- as.data.frame(Auto)

Auto.1 <- Auto.1 %>%
  mutate(cylinders = as.character(cylinders))

model.exp <- lm(mpg ~ year + origin + weight + horsepower + weight*horsepower + cylinders, data = Auto.1)
summary(model.exp)
```
as.factor could replace as.character for number variables

```{r tinkering with data 2}

model.exp2 <- lm(mpg ~ year + origin + weight + horsepower + weight*horsepower + as.factor(cylinders), data = Auto)
summary(model.exp2)
...
```{r sample1}
set.seed(1) #ensures we get the same set of randomly set data when we re-run the code
...
```{r differentseed}
set.seed(2) #using a different random number generator
train <- sample(392, 196)
lm.fit <- lm(mpg ~ horsepower, subset = train)
mean((mpg - predict(lm.fit, Auto))[-train]^2)
lm.fit2 <- lm(mpg ~ poly(horsepower, 2), data = Auto, #quadratic
    subset = train)
mean((mpg - predict(lm.fit2, Auto))[-train]^2)
lm.fit3 <- lm(mpg ~ poly(horsepower, 3), data = Auto, 
    subset = train)
...
```{r glm}
glm.fit <- glm(mpg ~ horsepower, data = Auto) #glm is the linear regression here
coef(glm.fit)
```
...
cv.err$delta #delta is the difference between model trained on full data vs. model trained on subset data, we should aim for the two models to be similar
```
